...
- **不确定时段单独记录。** 事件迟到超过可调整范围、队列溢出、前台无法识别时，记录不确定区间。
  已知的离开、锁屏和休眠不计入使用时长，也不会因此被称为缺失。
  缺口向外取整到秒，边界可能保守地少计一秒。
- **各个查询入口使用同一口径。** HTML 报表保留自包含页面的原有布局，普通记录缺口不会反复报警；
  实际读到损坏文件时，通过命令行说明读取问题。CSV/JSON 在原字段后追加 `record_type`，
  值为 `activity` 或 `gap`；缺口时长不算应用使用时长。
...
- **旧编号不可重用。** 启动先流式核验所有历史日志引用；已引用字典项丢失时保留原件并报错。
  ...
  会明确报错，避免静默漏扫历史引用。
- **启动失败保留原始原因。** `tracker run` 在控制台显示具体存储错误，无参数后台启动将其写入数据目录的
  `crash.log`。详见[启动错误复审](docs/startup-review.md)；`setup` 也会在执行安装前校验配置。
...
| 崩溃恢复 | 保留损坏原件，读取可验证前缀及编号续写文件；实际读取损坏文件时提供诊断 |
