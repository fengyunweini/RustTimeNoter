...
v0.2.0 可读取旧版数据，无需迁移。建议升级前备份完整数据目录。
恢复损坏日志时会保留原件并写入同目录的分片；备份时应保留这些分片和字典备份。
生成缺口或恢复分片后请使用新版查询，旧程序不理解这些记录约定。
CSV/JSON 增加 record_type；统计活动时筛选 activity，gap 表示未归属到应用的时间。
