...
// Tray cleanup must not consume the four-second session-ending flush window.
const TRAY_SHUTDOWN_TIMEOUT: Duration = Duration::from_millis(250);
...
impl TrayHandle {
    /// Request cleanup; detach after a short deadline if its window thread stalls.
...
fn finish_tray_thread(join: JoinHandle<()>, signal_delivered: bool) {
    // PostMessage success only confirms enqueueing. ShellExecute or another
    // window callback may still prevent dispatch, so every join is bounded.
    // If queueing failed, there is no reason to wait for an unrequested exit.
    let timeout = if signal_delivered {
        TRAY_SHUTDOWN_TIMEOUT
    } else {
        Duration::ZERO
    };
    ...
    while !join.is_finished() {
        ...
        if remaining.is_zero() {
            eprintln!("[rtn-tray] cleanup did not finish; detaching window thread");
            // Returning from main terminates the remaining thread; Windows
            // removes the icon even if shell cleanup is currently blocked.
            return;
        }
